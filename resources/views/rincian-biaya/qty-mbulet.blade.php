                                ...
                                <tr class="item">
                                    ...
                                    <td>
                                        <div class="input-group">
                                            ...
                                            <input type="number" name="subtotal_pengeluaran_riil[]" class="form-control subtotal_pengeluaran_riil">
                                        </div>
                                        </td>
                                </tr>
                                <tr id="jumlahnya">
                                    ...
                                    <td>
                                        <div class="input-group">
                                            <span class="input-group-addon">Rp</span>
                                            <input type="number" min="0" name="total_riil" class="form-control" readonly id="total_riil">
                                        </div>
                                    </td>
                                </tr>
                            </tbody>
            ...
            <form method="POST" action="{{url('rincian-biaya')}}" id="rForm" target="_blank">
                ...
                <tr>
                    ...
                    <td id="peng">
                        ...
                        {{-- <div class="col-md-6 padding-left-0">
                            <div>
                                <input type="number" class="form-control" name="qty_penginapan[]" min="0" required>
                            </div>
                        </div>
                        <div class="col-md-6 padding-left-0 padding-right-0">
                            <div class=>
                                <input type="text" class="form-control" name="harga_penginapan[]" placeholder="Harga" required>
                            </div>
                        </div> --}}
                    </td>
                    <td>
                ...
                <tr id="">
                    ...
                    <td>Daftar Pengeluaran Riil</td>
                    <td>
                        <br><br>
                        <input type="text" name="rill_subtotal" id="riil_subtotal" class="form-control" readonly>
                    </td>
                    <td>
                        <textarea name="keterangan" class="form-control" rows="4"></textarea>
                    </td>
                ...
                <table class="table">
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td style="border:none;">:</td>
                            <td style="border:none;"><input type="number" name="terbayar" class="form-control" style="width: 200px;"></td>
                        </tr>
                        <tr>
...
<script type="text/javascript">
    ...
    // $(document).on({
    //     change: function(){
    //         $(this).val(rupiahkan(($(this).val().length == 0) ? 0 : $(this).val()));
    //     },
    //     keyup:  function() {
    //         $(this).val(rupiahkan(($(this).val().length == 0) ? 0 : $(this).val()));
    //     }
    // }, '#tiket_pp, #airport_tax, #lainnya');
...
<script type="text/javascript">
    function tambahRiil(){
        ...
        var strVar="";
            ...
            strVar += "            <span class=\"input-group-addon\">Rp<\/span>";
            strVar += "            <input type=\"number\" min=\"0\" name=\"subtotal_pengeluaran_riil[]\" class=\"form-control subtotal_pengeluaran_riil\">";
            strVar += "        <\/div>";
            strVar += "        <\/td>";
            strVar += "<\/tr>";
    ...
    function recalculateRiil(){
        ...
        $('.subtotal_pengeluaran_riil').each(function(index, value){
            total += parseInt(($(this).val().length == 0) ? 0 : kembalikan($(this).val()));
        });
        $('#total_riil').val(total);
        // $('#total_riil').val(total);
        // return kembalikan(total;
    }
    ...
    $(document).on('click', '#simpanRiil', function() {
       $('.subtotal_pengeluaran_riil').each(function(index, value){
            if ($(this).val().length > 0 && $(this).val() > 0) {
                sendRiil('spd_id=' + $('input[name="spd_id"]').val() + '&jenis=' + $(this).parent().parent().find('.jenis_pengeluaran_riil').val() + '&sub_total=' + parseInt($(this).val()) + '&_token={{csrf_token()}}');
            }
        });
    });
        ...
        $.ajax({
            ...
            success : function(response) {
                ...
                if (response.status) {
                    ...
                    $('#riil_subtotal').val(recalculateRiil());
                    hitung_total();
                    $('#myModal').modal('hide');
                }else{
    ...
    $(document).on('change', '.jenis_pengeluaran_riil', function() {
       ...
       var maxPenginapan = parseInt(0.3 * {{$response['data']['referensi']['uang_taksi_asal']}});
       if (jenis == 'Biaya Riil Penginapan' && $(this).parent().parent().attr('class') == 'item') {
            $(this).parent().parent().find('.subtotal_pengeluaran_riil').attr('max', maxPenginapan).attr('data-placement', 'bottom').attr('data-toggle', 'popover').attr('data-content', 'Referensi Maksimal: {{$response['data']['referensi']['uang_taksi_asal']}} X 30% = ' + maxPenginapan);
            $(this).wrap('<div class="form-group"><div class="col-md-10"></div></div>');
            $(this).parent().parent().append('<div class="col-md-2"><input type="number" name="qty_riil[]" class="form-control qty_riil" min="0" step="1"></div>');
       ...
       // $('.jenis_pengeluaran_riil').each(function(k,v){
